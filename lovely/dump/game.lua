LOVELY_INTEGRITY = '516d850ba19957a631da28e12a672e7db70786c73624469129e6c7d8b524f748'
...
function Game:start_up()
    ...
    self:splash_screen()
    boot_timer('splash prep', 'end',1)    for _, key in ipairs(SMODS.Sticker.obj_buffer) do
...
function Game:set_language()
    ...
    if self.temp_language then self.SETTINGS.language = self.temp_language end
    self.LANG = self.LANGUAGES[self.SETTINGS.language] or self.LANGUAGES['en-us']

    local localization = self.LANG.full_path and NFS.getInfo(self.LANG.full_path) or love.filesystem.getInfo('localization/'..G.SETTINGS.language..'.lua')
    if localization ~= nil then
      self.localization = assert(loadstring(self.LANG.full_path and NFS.read(self.LANG.full_path) or love.filesystem.read('localization/'..G.SETTINGS.language..'.lua')))()
      init_localization()
    end
end
...
function Game:main_menu(change_context) --True if main menu is accessed from the splash screen, false if it is skipped or accessed from the game
    ...
    replace_card:set_edition('e_cry_glitched',true,true)
                                    ...
                                    if G.shop_abilities then
                                        if G.load_shop_abilities then 
                                            ...
                                            for i = 1, G.GAME.shop.ability_max - #G.shop_abilities.cards do
                                                ...
                                                local card = create_card('Ability', area, nil, nil, nil, nil, nil, 'sho')
                                                ...
                                                create_shop_card_ui(card, 'Ability', area)
                                                --card:start_materialize()
