LOVELY_INTEGRITY = '07bd300227cb8981af48c7422afe10aa7db1cedc69689ca1942c248db9f27c6b'
...
function Card:set_sprites(_center, _front)
    if _front then 
        if self.children.front then
            self.children.front.atlas = G.ASSET_ATLAS[G.SETTINGS.colourblind_option and _front.hc_atlas or _front.lc_atlas] or G.ASSET_ATLAS[_front.atlas] or G.ASSET_ATLAS["cards_"..(G.SETTINGS.colourblind_option and 2 or 1)]
            self.children.front:set_sprite_pos(self.config.card.pos)
        else
            self.children.front = Sprite(self.T.x, self.T.y, self.T.w, self.T.h, G.ASSET_ATLAS[G.SETTINGS.colourblind_option and _front.hc_atlas or _front.lc_atlas] or G.ASSET_ATLAS[_front.atlas] or G.ASSET_ATLAS["cards_"..(G.SETTINGS.colourblind_option and 2 or 1)], self.config.card.pos)
        ...
        if _center.set then
            if self.children.center then
                ...
                elseif not self.params.bypass_discovery_center and (_center.set == 'Edition' or _center.set == 'Joker' or _center.consumeable or _center.set == 'Voucher' or _center.set == 'Booster') and not _center.discovered then 
                    self.children.center = Sprite(self.T.x, self.T.y, self.T.w, self.T.h, G.ASSET_ATLAS[SMODS.UndiscoveredSprites[_center.set] and (SMODS.UndiscoveredSprites[_center.set][G.SETTINGS.colourblind_option and 'hc_atlas' or 'lc_atlas'] or SMODS.UndiscoveredSprites[_center.set].atlas) or _center.set], 
...
function Card:set_ability(center, initial, delay_sprites)
    ...
    if not next(SMODS.find_card(old_center.key, true)) then
...
function Card:set_cost()
    ...
    if self.misprint_cost_fac then self.cost = cry_format(self.cost * self.misprint_cost_fac,'%.2f') 
...
function Card:add_to_deck(from_debuff)
    ...
    if not self.added_to_deck then
        ...
        end        local obj = self.config.center
        ...
        if G.GAME.blind and not (Talisman.config_file.disable_anims and (Talisman.calculating_joker or Talisman.calculating_score or Talisman.calculating_card)) then G.E_MANAGER:add_event(Event({ func = function() G.GAME.blind:set_blind(nil, true, nil); return true end })) end
    end
end

function Card:remove_from_deck(from_debuff)
    if self.added_to_deck then
        self.added_to_deck = false        local obj = self.config.center
...
function Card:generate_UIBox_ability_table()
    ...
    if self.base.colour then self.base.colour = G.C.SUITS[self.config.card.suit] end
...
function Card:can_use_consumeable(any_state, skip_check)
    ...
    if G.STATE ~= G.STATES.HAND_PLAYED and G.STATE ~= G.STATES.DRAW_TO_HAND and G.STATE ~= G.STATES.PLAY_TAROT or any_state then
        ...
        if G.STATE == G.STATES.SELECTING_HAND or G.STATE == G.STATES.TAROT_PACK or G.STATE == G.STATES.SPECTRAL_PACK or G.STATE == G.STATES.PLANET_PACK or G.STATE == G.STATES.SMODS_BOOSTER_OPENED then
            if self.ability.consumeable.max_highlighted then
                if (self.ability.consumeable.mod_num - (G.GAME.modifiers.cry_consumable_reduce and (self.ability.consumeable.mod_num > 1) and 1 or 0)) >= #G.hand.highlighted + (self.area == G.hand and -1 or 0) and #G.hand.highlighted + (self.area == G.hand and -1 or 0) >= 1 then
                    return true
                end
            end
        ...
        if context.joker_main then
            if G.GAME.used_vouchers.v_observatory and self.ability.consumeable.hand_type == context.scoring_name then
                return {
                    ...
                    Xmult_mod = G.P_CENTERS.v_observatory.config.extra^(self.ability.qty or 1)
                }
            end
        end
    end
...
function Card:draw(layer)
    ...
    if (layer == 'card' or layer == 'both') then
        ...
        elseif self.sprite_facing == 'front' then 
            ...
            elseif not self.greyed and not (get_betmma_shaders and get_betmma_shaders(self)) then 
            ...
            if self.edition or self.seal or self.ability.eternal or self.ability.rental or self.ability.perishable or self.sticker or self.ability.set == 'Spectral' or self.debuff or self.greyed or self.ability.name == 'The Soul' or self.ability.set == 'Voucher' or self.ability.set == 'Booster' or self.config.center.soul_pos or self.config.center.demo then
                ...
                if self.seal then
                    ...
                    if obj.get_p_dollars and type(obj.get_p_dollars) == 'function' then
                        ret = ret + obj.get_p_dollars(self)
                    elseif self.seal == 'Gold' then G.shared_seals[self.seal]:draw_shader('voucher', nil, self.ARGS.send_to_shader, nil, self.children.center) end
                end
                if self.ability.eternal then
                    ...
                    if self.edition then 
                        for k, v in pairs(G.P_CENTER_POOLS.Edition) do
                            if v.apply_to_float then
                                ...
                                if self.ability.name == 'cry-gateway_ex' and (self.config.center.discovered or self.bypass_discovery_center) then
                                    local scale_mod2 = 0.07-- + 0.02*math.cos(1.8*G.TIMERS.REAL) + 0.00*math.cos((G.TIMERS.REAL - math.floor(G.TIMERS.REAL))*math.pi*14)*(1 - (G.TIMERS.REAL - math.floor(G.TIMERS.REAL)))^3
                                    local rotate_mod2 = 0--0.05*math.cos(1.219*G.TIMERS.REAL) + 0.00*math.cos((G.TIMERS.REAL)*math.pi*5)*(1 - (G.TIMERS.REAL - math.floor(G.TIMERS.REAL)))^2
                                    self.children.floating_sprite2:draw_shader('dissolve',0, nil, nil, self.children.center,scale_mod2, rotate_mod2,nil, 0.1--[[ + 0.03*math.cos(1.8*G.TIMERS.REAL)--]],nil, 0.6)
                                    self.children.floating_sprite2:draw_shader('dissolve', nil, nil, nil, self.children.center, scale_mod2, rotate_mod2) 
        ...
        if self.ability.banana then
            G.shared_sticker_banana.role.draw_major = self
            G.shared_sticker_banana:draw_shader('dissolve', nil, nil, nil, self.children.center)
            G.shared_sticker_banana:draw_shader('voucher', nil, self.ARGS.send_to_shader, nil, self.children.center)
        end
