...
extern MY_HIGHP_OR_MEDIUMP vec2 kraken;
...
vec4 effect( vec4 colour, Image texture, vec2 texture_coords, vec2 screen_coords )
{
	...
	float t = kraken.y*2.221 + real_time*99 + time*199;
    ...
    float field = (1.+ (
        cos(length(field_part1) / 19.483) + sin(length(field_part2) / 33.155) * cos(field_part2.y / 15.73) +
        cos(length(field_part3) / 27.193) * sin(field_part3.x / 21.92) ))/2.;
	vec2 mouse_offset = (mouse_screen_pos.xy )/screen_scale; // want to add krakens following mouse but failed
	...
	float ray=cos(2*angle);//-mod(real_time, .2)
	float ray4=ray * ray ;
