...
do
    ...
    local Card_calculate_joker_ref=Card.calculate_joker
    function Card:calculate_joker(context)
        if self.ability.set == "Joker" and not self.debuff and self.edition and (self.edition.tentacle or self.edition.kraken or self.edition.cthulhu) and not context.blueprint and context.setting_blind and not self.getting_sliced then 
            local my_pos = nil
            for i = 1, #G.jokers.cards do
                ...
                G.E_MANAGER:add_event(Event({func = function()
                    ...
                    for k, v in pairs(possibleKeys) do
                        if self_ability[v] and self_ability[v]~=0 then
                            if self.edition.tentacle then
                                self_ability[v]=self_ability[v]+values[valueIndex]
			    end
                            if self.edition.kraken then
                                self_ability[v]=self_ability[v] * values[valueIndex]
			    end
                            if self.edition.cthulhu then
                                self_ability[v]=self_ability[v] ^ values[valueIndex]
			    end
